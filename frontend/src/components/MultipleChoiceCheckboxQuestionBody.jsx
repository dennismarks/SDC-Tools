...
export default class MultipleChoiceCheckboxQuestionBody extends Component {
    ...
    if (props.answer) {
      for (let i = 0; i < props.question_options.length; i++) {
        ...
        if (props.answer.indexOf(option.optionID) > -1) {
          this.state["checked_options"][option.optionID] = true;
        }
        else {
          this.state["checked_options"][option.optionID] = false;
        }
      }
    }
  }
  ...
  handleChange(event) {
    ...
    for (let i = 0; i < this.props.question_options.length; i++) {
      ...
      if (this.state["checked_options"][option.optionID]) {
    ...
    for (let i = 0; i < this.props.question_options.length; i++) {
      ...
      components.push(
        <div style={{marginBottom: "3px"}}>
          <input type="checkbox" id={this.props.question_id + option.optionID} name={this.props.question_id} value={option.optionID} onChange={this.handleChange} checked={this.state["checked_options"][option.optionID]} />
